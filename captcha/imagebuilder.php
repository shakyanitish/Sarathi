...
$white = imagecolorallocate($image, $bgRed, $bgGreen, $bgBlue);
...
for ($i = 0; $i < $stringlength; $i++) {
	...
	// Create character
	imagestring($buffer, 5, 5, 5, $cnum[$i], $color);

	// Resize character
	imagecopyresized ($buffer2, $buffer, 0, 0, 0, 0, 25 + mt_rand(0,12), 25 + mt_rand(0,12), 20, 20);

	if($rotate_string == 1) $rotate_angle = mt_rand(-25, 25);

	// Rotate characters a little
	$rotated = imagerotate($buffer2, $rotate_angle,imagecolorallocatealpha($buffer2,0,0,0,0)); 
