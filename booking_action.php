...
if ($_POST['action'] == "forbooking"):
	...
	foreach ($roomprice as $k => $v) {
		foreach ($roomprice[$k] as $rk => $rv) {
			$newArr[] = array('roomname' => $k, 'ppqnty' => $ppqnty[$k][$rk], 'roomprice' => $rv, 'roomqnty' => $roomqnty[$k][$rk], 'extrabed' => $extrabed[$k][$rk], 'extrabedrate' => $extrabedrate[$k][$rk]);
		}
	}
	...
	foreach ($newArr as $reck => $recv) {
		if ($recv['roomqnty'] != 'N/A') {
			$body .= '<tr style="background: #F1F1F1;">
							...
							<td style="padding: 8px 10px; text-align:center;">' . $recv['roomprice'] . '</td>
