...
class Package extends DatabaseObject {
	...
	protected static $db_fields = array('id', 'slug', 'title', 'sub_title', 'brief', 'content', 'gallery', 'status', 'sortorder', 'meta_keywords', 'meta_description', 'type', 'added_date', 'homepage', 'image');
	...
	public static function get_history_list()
	{
		global $db;
		$sql = "SELECT slug, title, content, image FROM ".self::$table_name." WHERE status='1' AND type='2' ORDER BY sortorder DESC ";
		$result = self::find_by_sql($sql);
		return $result;
	}

	public static function get_complex_list()
	{
		global $db;
		$sql = "SELECT slug, title, content, image FROM ".self::$table_name." WHERE status='1' AND type='3' ORDER BY sortorder DESC ";
		$result = self::find_by_sql($sql);
		return $result;
	}
	...
	// Homepage Display
	public static function homepagePackage($limit=''){
		global $db;
	...
	// Package display
	public static function getPackage($page=""){
		global $db;
	...
	{
		global $db;		
		$sql = "SELECT slug, title, type FROM ".self::$table_name." WHERE status='1' ORDER BY sortorder ASC";
		...
		$result.='<optgroup label="Packages">';			
			foreach($pages as $pageRow):
				if($pageRow->type=='1') { $pglink ='page/'.$pageRow->slug; }				
				if($pageRow->type=='3') { $pglink ='complex/'.$pageRow->slug; }
				$sel = ($Lsel==$pglink) ?'selected':'';
				if($pageRow->type!='2') {
				$result.='<option value="'.$pglink.'" '.$sel.'>&nbsp;&nbsp;'.$pageRow->title.'</option>';
				}
			endforeach;
		$result.='</optgroup>';	
		endif;
