...
class Article extends DatabaseObject
{
    ...
    protected static $db_fields = array('id', 'slug', 'title', 'sub_title', 'linktype', 'linksrc', 'brief', 'content', 'status', 'sortorder', 'meta_title', 'meta_keywords', 'meta_description', 'type', 'added_date', 'homepage', 'image', 'fb_upload');
    ...
    public static function find_by_type($type = 0, $lmt = '')
    {
        global $db;
        $limit = !empty($lmt) ? ' LIMIT ' . $lmt : '';
        $sql = "SELECT * FROM " . self::$table_name . " WHERE type={$type} AND status='1' ORDER BY sortorder DESC $limit ";
    ...
    public static function get_internal_link($Lsel = '', $LType = 0)
    {
        ...
        if ($pages):
            ...
            foreach ($pages as $pageRow):
                $sel = ($Lsel == $pageRow->linksrc) ? 'selected' : '';
