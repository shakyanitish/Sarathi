...
function CroppedThumbnail($imgSrc, $thumbnail_width, $thumbnail_height, $dest)
{
    ...
    if ($img_type == 1 || $img_type == 2 || $img_type == 3 || $img_type == 18) {
        if ($img_type == 1) $myImage = imagecreatefromgif($imgSrc);
        elseif ($img_type == 2) $myImage = imagecreatefromjpeg($imgSrc);
        elseif ($img_type == 3) $myImage = imagecreatefrompng($imgSrc);
        else $myImage = imagecreatefromwebp($imgSrc);
...
if (!empty($_FILES)) {
    ...
    $file_ext = explode('.', $targetFile)[1]; // taking the extension of uploaded img
    ...
    $fileTypes = array('jpg', 'jpeg', 'webp'); // File extensions
    ...
    if (in_array(strtolower($fileParts['extension']), $fileTypes)) {
        {
            if ($fileTypes == 'webp') {
                move_uploaded_file($tempFile, $targetPath . str_replace($file_ext, 'webp', $targetFile));
            } else {
                move_uploaded_file($tempFile, $targetPath . $targetFile);
            }
            if ($fileParts['extension'] == 'svg') {
                copy($targetPath . $targetFile, $targetFileThumbs);
            }
            if (in_array(strtolower($fileParts['extension']), array('jpg', 'jpeg', 'gif', 'png', 'webp'))) {
